...
<Container maxWidth="lg">
    <NavLogo><h1>    كلية علوم ذوي الاحتياجات الخاصة  </h1></NavLogo>
    ...
    <NavLinks>
        ...
        <YouTubeIcon className="icons fl"  style={{color: "rgb(255 17 0 / 80%)"}}/>
      ...
      {navData.map((nav,index)=>(
        <Link  activeStyle={{ color: "red" }}  to={nav.link} key={index}>
          <ListItem button style={{paddingRight: 36,paddingLeft: 50}}>
            ...
            <ListItemText primary={nav.name} style={{fontWeight: 'bold' , color: 'black'}}/>
          </ListItem>
        </Link>
      ))}
   </List>
...
const NavLinks = styled.nav`
  ...
  .m_up { display: inline-block;}
  .m_down {display: none;}

  @media only screen and (max-width: 970px) {

     .m_up { display: none; }
     .m_down { display: inline-block;}
     .icons{ margin: 4px 10px; font-size: 1.8rem; }
